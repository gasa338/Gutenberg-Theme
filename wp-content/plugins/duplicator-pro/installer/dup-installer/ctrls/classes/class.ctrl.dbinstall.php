    ...
    protected static function skipQuery($query)
    {
        ...
        if (is_null($skipRegex)) {
            ...
            if (count($skipCreate) > 0) {
                ...
                for ($i = 0; $i < ceil(count($skipCreate) / self::SKIP_TABLES_REGEX_CHUNK_SIZE); $i++) {
                    $subArray = array_slice($skipCreate, $i * self::SKIP_TABLES_REGEX_CHUNK_SIZE, self::SKIP_TABLES_REGEX_CHUNK_SIZE);
    ...
    private static function rulesTableNames()
    {
        ...
        foreach ($mapping as $oldPrefix => $newMapping) {
            ...
            foreach ($newMapping as $newPrefix => $commons) {
                Log::info('CHANGHE TABLES PREFIX ' . $oldPrefix . ' TO ' . $newPrefix . 'FOR ' . Log::v2str($commons));
